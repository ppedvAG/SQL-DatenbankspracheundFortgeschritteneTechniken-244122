...
select orderid, employeeid, customerid, freight, 
		ROW_NUMBER() over
					(
						partition by employeeid order by freight desc
					) Rang
from orders
--where rang = 1
...
select employeeid, cume_dist() over (partition by employeeid 
						order by freight),
					percent_rank() over (partition by employeeid 
						order by freight)
from orders
...
--EIn paar Schmankerl zum probieren

-------------------------------------------------------------------------------------
--Gibt es Produkte , die im aktuellen Monat (1998 Monat) 3 	h�tten nachbestellt werden sollen 
...
group by productname  order by 2 desc
 ...
 -------------------------------------------------------------------------------------
 --Pro Angestellte Ver�nderung der Lieferkosten gegen�ber Vorjahr und  
  -------------------------------------------------------------------------------------
...
--�bersicht nach Diff zum Vorjahr
